...
var URL = 'http://%s.search.yahoo.com/search?p=%s&b=%d&n=%d&ie=UTF-8&oe=UTF-8';
...
function yahoo(query, sedb, lang, start, proxy, device, callback) {
    if (typeof callback === 'undefined') {
        callback = start
    } else {
        startIndex = start
    }
    iyahoo(query, sedb, lang, 0, proxy, [], [], 0, device, callback);
}
...
var iyahoo = function (query, sedb, lang, start, proxy, links, urls, index, device, callback) {
    ...
    request(requestOptions, function (err, resp, body) {
        if ((err == null) && resp.statusCode === 200) {
            ...
            $(itemSelector).each(function (i, elem) {
                ...
                if(qsObj['http://ri.search.yahoo.com/_ylt']) {
                    links.push(Helpers.getDomainURL(qsObj['http://ri.search.yahoo.com/_ylt']));
                    urls.push(Helpers.getSubdomainURL(qsObj['http://ri.search.yahoo.com/_ylt']));
                } else if (qsObj['/url?q']) {
            ...
            if (links.length < yahoo.totalResults && index < 5) {
                ...
                index++;
                iyahoo(query, sedb, lang, start + yahoo.resultsPerPage + 1, proxy, links, urls, index, device, callback)
            } else {
