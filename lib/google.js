...
var URL = '%s://www.google.%s/search?hl=%s&q=%s&start=%s&sa=N&num=%s&ie=UTF-8&oe=UTF-8&gws_rd=ssl&gl=%s&uule=%s';
...
var igoogle = function(query, sedb, lang, start, proxy, links, urls, index, callback) {
        ...
        request(requestOptions, function(err, resp, body) {
            if ((err == null) && resp.statusCode === 200) {
                ...
                if (links.length < google.totalResults && index < 2) {
                    ...
                    index++;
                    igoogle(query, sedb, lang, start + google.resultsPerPage, proxy, links, urls, index, callback)
                } else {
