...
function google(query, sedb, lang, start, proxy, device, cert, callback) {
    if (typeof callback === 'undefined') {
        callback = start
    } else {
        startIndex = start
    }
    igoogle(query, sedb, lang, 0, proxy, [], [], 0, device, cert, callback);
}
...
var igoogle = function(query, sedb, lang, start, proxy, links, urls, index, device, cert, callback) {
        ...
        request(requestOptions, function(err, resp, body) {
            if ((err == null) && resp.statusCode === 200) {
                ...
                if (links.length < google.totalResults && index < 2) {
                    ...
                    index++;
                    igoogle(query, sedb, lang, start + google.resultsPerPage, proxy, links, urls, index, device, cert, callback)
                } else {
