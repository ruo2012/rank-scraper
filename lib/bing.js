...
function bing(query, sedb, lang, start, proxy, device, callback) {
    if (typeof callback === 'undefined') {
        callback = start
    } else {
        startIndex = start
    }
    ibing(query, sedb, lang, 0, proxy, [], [], 0, device, callback);
}
...
var ibing = function (query, sedb, lang, start, proxy, links, urls, index, device, callback) {
    ...
    request(requestOptions, function (err, resp, body) {
        if ((err == null) && resp.statusCode === 200) {
            ...
            if (links.length < bing.totalResults && index < 3) {
                ...
                index++;
                ibing(query, sedb, lang, start + bing.resultsPerPage + 1, proxy, links, urls, index, device, callback)
            } else {
